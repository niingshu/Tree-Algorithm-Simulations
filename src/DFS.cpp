...
vector<int> DFS::traverse(const Graph& graph, int start, EventRecord& recorder) {
    ...
    while (!s.empty()) {
        ...
        if (!visited[v]) { //havent visited yet
            ...
            recorder.record(Algorithm::DFS, Action::VISIT_NODE, v); //VISIT_NODE
        }
        ...
        for (auto it = graph.getNeighbours(v).rbegin(); it != graph.getNeighbours(v).rend(); ++it) {
            ...
            recorder.record (Algorithm::DFS, Action::DISCOVERY_EDGE, v, neighbor, weight);

            if (!visited[neighbor]) { //if havent visited then push, if not skip
                s.push(neighbor);
                recorder.record(Algorithm::DFS, Action::ADD_TO_TREE, v, neighbor, weight);
            } else {
                recorder.record(Algorithm::DFS, Action::RELAX_EDGE, v, neighbor, weight);
            }
        }
    }
